#ifndef __ORANGE_STL_ALGO_H__
#define __ORANGE_STL_ALGO_H__
...
{
  auto d1 = orange_stl::distance(first1, last1);
  auto d2 = orange_stl::distance(first2, last2);
  if (d1 < d2)
    return last1;
...
search_n(ForwardIter first, ForwardIter last, Size n, const T& value)
{
  ...
  {
    first = orange_stl::find(first, last, value);
    while (first != last)
    {
      auto m = n - 1;
      auto i = first;
      ...
      {
        first = orange_stl::find(i, last, value);
      }
    }
    return last;
  }
                  ...
                  forward_iterator_tag, forward_iterator_tag)
{
  ...
  else
  {
    ...
    while (true)
    {
      ...
      auto new_result = orange_stl::search(first1, last1, first2, last2);
      if (new_result == last1)
      {
        return result;
      }
                  ...
                  bidirectional_iterator_tag, bidirectional_iterator_tag)
{
  ...
  reviter1 rresult = orange_stl::search(reviter1(last1), rlast1, reviter2(last2), rlast2);
  if (rresult == rlast1)
  {
    return last1;
  }
  else
  {
    ...
    orange_stl::advance(result, -orange_stl::distance(first2, last2));
    return result;
  }
}
         ...
         ForwardIter2 first2, ForwardIter2 last2)
{
  ...
  return orange_stl::find_end_dispatch(first1, last1, first2, last2, Category1(), Category2());
}
                  ...
                  forward_iterator_tag, forward_iterator_tag, Compared comp)
{
  ...
  else
  {
    ...
    {
      // 利用 search 查找某个子序列的首次出现点，找不到则返回 last1
      auto new_result = orange_stl::search(first1, last1, first2, last2, comp);
      if (new_result == last1)
      {
        return result;
      }
                  ...
                  bidirectional_iterator_tag, bidirectional_iterator_tag, Compared comp)
{
  ...
  reviter1 rlast1(first1);
  reviter2 rlast2(first2);
  reviter1 rresult = orange_stl::search(reviter1(last1), rlast1, reviter2(last2), rlast2, comp);
  if (rresult == rlast1)
  {
    return last1;
  }
         ...
         ForwardIter2 first2, ForwardIter2 last2, Compared comp)
{
  typedef typename iterator_traits<ForwardIter1>::iterator_category Category1;
  typedef typename iterator_traits<ForwardIter2>::iterator_category Category2;
  return orange_stl::find_end_dispatch(first1, last1, first2, last2, Category1(), Category2(), comp);
}
...
{
  auto len = orange_stl::distance(first, last);
  auto half = len;
  ForwardIter middle;
  while (len > 0)
  {
    half = len >> 1;
    middle = first;
    orange_stl::advance(middle, half);
    if (*middle < value)
    {
      first = middle;
      ++first;
...
{
  return orange_stl::lbound_dispatch(first, last, value, iterator_category(first));
}
...
{
  return orange_stl::lbound_dispatch(first, last, value, iterator_category(first), comp);
}
...
{
  return orange_stl::ubound_dispatch(first, last, value, iterator_category(first));
}
...
{
  return orange_stl::ubound_dispatch(first, last, value, iterator_category(first), comp);
}
...
{
  auto i = orange_stl::lower_bound(first, last, value);
...
template <class ForwardIter, class T>
orange_stl::pair<ForwardIter, ForwardIter>
                ...
                const T& value, forward_iterator_tag)
{
  ...
  while (len > 0)
  {
    ...
    {
      left = orange_stl::lower_bound(first, last, value);
      orange_stl::advance(first, len);
      right = orange_stl::upper_bound(++middle, first, value);
      return orange_stl::pair<ForwardIter, ForwardIter>(left, right);
    }
  }
  return orange_stl::pair<ForwardIter, ForwardIter>(last, last);
}

// erange_dispatch 的 random_access_iterator_tag 版本
template <class RandomIter, class T>
orange_stl::pair<RandomIter, RandomIter>
                ...
                const T& value, random_access_iterator_tag)
{
  ...
  while (len > 0)
  {
    ...
    {
      left = orange_stl::lower_bound(first, middle, value);
      right = orange_stl::upper_bound(++middle, first + len, value);
      return orange_stl::pair<RandomIter, RandomIter>(left, right);
    }
  }
  return orange_stl::pair<RandomIter, RandomIter>(last, last);
}
...
{
  return orange_stl::erange_dispatch(first, last, value, iterator_category(first));
}
                ...
                const T& value, forward_iterator_tag, Compared comp)
{
  ...
  while (len > 0)
  {
    ...
    {
      left = orange_stl::lower_bound(first, last, value, comp);
      orange_stl::advance(first, len);
      right = orange_stl::upper_bound(++middle, first, value, comp);
      return orange_stl::pair<ForwardIter, ForwardIter>(left, right);
    }
  }
  return orange_stl::pair<ForwardIter, ForwardIter>(last, last);
                ...
                const T& value, random_access_iterator_tag, Compared comp)
{
  ...
  while (len > 0)
  {
    ...
    {
      left = orange_stl::lower_bound(first, middle, value, comp);
      right = orange_stl::upper_bound(++middle, first + len, value, comp);
      return orange_stl::pair<RandomIter, RandomIter>(left, right);
    }
  }
  return orange_stl::pair<RandomIter, RandomIter>(last, last);
...
{
  return orange_stl::erange_dispatch(first, last, value, iterator_category(first), comp);
}
...
{
  auto n = orange_stl::distance(first, last);
  auto parent = 0;
            ...
            ForwardIter2 first2)
{
  ...
  {
    orange_stl::iter_swap(first1, first2);
  }
  return first2;
}
...
{
  first = orange_stl::find(first, last, value);  // 利用 find 找出第一个匹配的地方
  auto next = first;
  return first == last ? first : orange_stl::remove_copy(++next, last, first, value);
}
...
{
  first = orange_stl::find_if(first, last, unary_pred);  // 利用 find_if 找出第一个匹配的地方
  auto next = first;
  return first == last ? first : orange_stl::remove_copy_if(++next, last, first, unary_pred);
}
                      ...
                      bidirectional_iterator_tag)
{
  while (true)
  {
    if (first == last || first == --last)
      return;
    orange_stl::iter_swap(first++, last);
  }
}
...
{
  orange_stl::reverse_dispatch(first, last, iterator_category(first));
}
...
void random_shuffle(RandomIter first, RandomIter last)
{
  ...
  {
    orange_stl::iter_swap(i, first + (rand() % (i - first + 1)));
  }
}
                    ...
                    RandomNumberGenerator& rand)
{
  ...
  for (auto i = first + 1; i != last; ++i)
  {
    orange_stl::iter_swap(i, first + (rand(i - first + 1) % len));
  }
}
...
{
  orange_stl::reverse_dispatch(first, middle, bidirectional_iterator_tag());
  orange_stl::reverse_dispatch(middle, last, bidirectional_iterator_tag());
  while (first != middle && middle != last)
    orange_stl::swap(*first++, *--last);
  if (first == middle)
  {
    orange_stl::reverse_dispatch(middle, last, bidirectional_iterator_tag());
    return last;
                ...
                RandomIter last, random_access_iterator_tag)
{
  ...
  if (l == r)
  {
    orange_stl::swap_ranges(first, middle, last);
    return result;
  }
...
rotate(ForwardIter first, ForwardIter middle, ForwardIter last)
{
  ...
  if (middle == last)
    return first;
  return orange_stl::rotate_dispatch(first, middle, last, iterator_category(first));
}
...
{
  return orange_stl::copy(first, middle, orange_stl::copy(middle, last, result));
}
...
bool is_permutation_aux(ForwardIter1 first1, ForwardIter1 last1,
                        ...
                        BinaryPred pred)
{
  constexpr bool is_ra_it = orange_stl::is_random_access_iterator<ForwardIter1>::value
    && orange_stl::is_random_access_iterator<ForwardIter2>::value;
  if (is_ra_it)
  {
  ...
  {
    auto len1 = orange_stl::distance(first1, last1);
    auto len2 = orange_stl::distance(first2, last2);
    if (len1 == 0 && len2 == 0)
      return true;
    if (len1 != len2)
      return false;
                    ...
                    ForwardIter2 first2, ForwardIter2 last2)
{
  ...
  static_assert(std::is_same<v1, v2>::value,
                "the type should be same in orange_stl::is_permutation");
  return is_permutation_aux(first1, last1, first2, last2,
                            orange_stl::equal_to<v1>());
}
...
bool next_permutation(BidirectionalIter first, BidirectionalIter last)
{
  ...
  for (;;)
  {
    ...
    {                 // 找到第一对小于关系的元素
      auto j = last;
      while (!(*i < *--j)) {}
      orange_stl::iter_swap(i, j);       // 交换 i，j 所指元素
      orange_stl::reverse(ii, last);     // 将 ii 之后的所有元素反转
      return true;
    }
    if (i == first)
    {
      orange_stl::reverse(first, last);
      return false;
    }
  }
}
...
bool prev_permutation(BidirectionalIter first, BidirectionalIter last)
{
  ...
  for (;;)
  {
    ...
    {                 // 找到第一对大于关系的元素
      auto j = last;
      while (!(*--j < *i)) {}
      orange_stl::iter_swap(i, j);       // 交换i，j
      orange_stl::reverse(ii, last);     // 将 ii 之后的所有元素反转
      return true;
    }
    if (i == first)
      ...
      OutputIter result)
{
  ...
  return orange_stl::copy(first2, last2, orange_stl::copy(first1, last1, result));
}
                          ...
                          BidirectionalIter last, Distance len1, Distance len2)
{
  ...
  {
    if (*middle < *first)
      orange_stl::iter_swap(first, middle);
    return;
  }
  ...
  {  // 序列一较长，找到序列一的中点
    len11 = len1 >> 1;
    orange_stl::advance(first_cut, len11);
    second_cut = orange_stl::lower_bound(middle, last, *first_cut);
    len22 = orange_stl::distance(middle, second_cut);
  }
  else
  {              // 序列二较长，找到序列二的中点
    len22 = len2 >> 1;
    orange_stl::advance(second_cut, len22);
    first_cut = orange_stl::upper_bound(first, middle, *second_cut);
    len11 = orange_stl::distance(first, first_cut);
  }
  auto new_middle = orange_stl::rotate(first_cut, middle, second_cut);
  orange_stl::merge_without_buffer(first, first_cut, new_middle, len11, len22);
  orange_stl::merge_without_buffer(new_middle, second_cut, last, len1 - len11, len2 - len22);
}
...
{
  if (first1 == last1)
    return orange_stl::copy_backward(first2, last2, result);
  if (first2 == last2)
    return orange_stl::copy_backward(first1, last1, result);
  --last1;
  --last2;
  while (true)
  {
                ...
                BidirectionalIter2 buffer, Distance buffer_size)
{
  ...
  {
    buffer_end = orange_stl::copy(middle, last, buffer);
    orange_stl::copy_backward(first, middle, last);
    return orange_stl::copy(buffer, buffer_end, first);
  }
  else if (len1 <= buffer_size)
  {
    buffer_end = orange_stl::copy(first, middle, buffer);
    orange_stl::copy(middle, last, first);
    return orange_stl::copy_backward(buffer, buffer_end, last);
  }
  else
  {
    return orange_stl::rotate(first, middle, last);
  }
}
                    ...
                    Pointer buffer, Distance buffer_size)
{
  ...
  {
    Pointer buffer_end = orange_stl::copy(first, middle, buffer);
    orange_stl::merge(buffer, buffer_end, middle, last, first);
  }
  else if (len2 <= buffer_size)
  {
    Pointer buffer_end = orange_stl::copy(middle, last, buffer);
    orange_stl::merge_backward(first, middle, buffer, buffer_end, last);
  }
  else
  {  // 区间长度太长，分割递归处理
    auto first_cut = first;
    ...
    auto new_middle = orange_stl::rotate_adaptive(first_cut, middle, second_cut,
                                             len1 - len11, len22, buffer, buffer_size);
    orange_stl::merge_adaptive(first, first_cut, new_middle, len11, len22, buffer, buffer_size);
    orange_stl::merge_adaptive(new_middle, second_cut, last, len1 - len11,
...
{
  auto len1 = orange_stl::distance(first, middle);
  auto len2 = orange_stl::distance(middle, last);
  temporary_buffer<BidirectionalIter, T> buf(first, last);
  if (!buf.begin())
  {
    orange_stl::merge_without_buffer(first, middle, last, len1, len2);
  }
  else
  {
    orange_stl::merge_adaptive(first, middle, last, len1, len2, buf.begin(), buf.size());
  }
}
              ...
              BidirectionalIter last)
{
  if (first == middle || middle == last)
    return;
  orange_stl::inplace_merge_aux(first, middle, last, value_type(first));
}
                          ...
                          Compared comp)
{
  ...
  if (len1 > len2)
  {
    len11 = len1 >> 1;
    orange_stl::advance(first_cut, len11);
    second_cut = orange_stl::lower_bound(middle, last, *first_cut, comp);
    len22 = orange_stl::distance(middle, second_cut);
  }
  else
  {
    len22 = len2 >> 1;
    orange_stl::advance(second_cut, len22);
    first_cut = orange_stl::upper_bound(first, middle, *second_cut, comp);
    len11 = orange_stl::distance(first, first_cut);
  }
  auto new_middle = orange_stl::rotate(first_cut, middle, second_cut);
  orange_stl::merge_without_buffer(first, first_cut, new_middle, len11, len22, comp);
  orange_stl::merge_without_buffer(new_middle, second_cut, last, len1 - len11, len2 - len22, comp);
}
                    ...
                    Pointer buffer, Distance buffer_size, Compared comp)
{
  ...
  {
    Pointer buffer_end = orange_stl::copy(first, middle, buffer);
    orange_stl::merge(buffer, buffer_end, middle, last, first, comp);
  }
  else if (len2 <= buffer_size)
  {
    Pointer buffer_end = orange_stl::copy(middle, last, buffer);
    orange_stl::merge_backward(first, middle, buffer, buffer_end, last, comp);
  }
  else
  {  // 区间长度太长，分割递归处理
    auto first_cut = first;
    ...
    auto new_middle = orange_stl::rotate_adaptive(first_cut, middle, second_cut, len1 - len11,
                                             len22, buffer, buffer_size);
    orange_stl::merge_adaptive(first, first_cut, new_middle, len11,
                  ...
                  BidirectionalIter last, T*, Compared comp)
{
  ...
  if (!buf.begin())
  {
    orange_stl::merge_without_buffer(first, middle, last, len1, len2, comp);
  }
  else
  {
    orange_stl::merge_adaptive(first, middle, last, len1, len2, buf.begin(), buf.size(), comp);
  }
}
              ...
              BidirectionalIter last, Compared comp)
{
  if (first == middle || middle == last)
    return;
  orange_stl::inplace_merge_aux(first, middle, last, value_type(first), comp);
}
...
void partial_sort(RandomIter first, RandomIter middle,
                  RandomIter last)
{
  orange_stl::make_heap(first, middle);
  for (auto i = middle; i < last; ++i)
  {
    if (*i < *first)
    {
      orange_stl::pop_heap_aux(first, middle, i, *i, distance_type(first));
    }
  }
  orange_stl::sort_heap(first, middle);
}
...
{
  orange_stl::make_heap(first, middle, comp);
  for (auto i = middle; i < last; ++i)
  {
    if (comp(*i, *first))
    {
      orange_stl::pop_heap_aux(first, middle, i, *i, distance_type(first), comp);
    }
  }
  orange_stl::sort_heap(first, middle, comp);
}
               ...
               Distance*)
{
  ...
  orange_stl::make_heap(result_first, result_iter);
  while (first != last)
  {
    if (*first < *result_first)
    {
      orange_stl::adjust_heap(result_first, static_cast<Distance>(0),
                           result_iter - result_first, *first);
    }
    ++first;
  }
  orange_stl::sort_heap(result_first, result_iter);
  return result_iter;
}
...
partial_sort_copy(InputIter first, InputIter last,
                  RandomIter result_first, RandomIter result_last)
{
  return orange_stl::psort_copy_aux(first, last, result_first, result_last,
               ...
               Distance*, Compared comp)
{
  ...
  orange_stl::make_heap(result_first, result_iter, comp);
  while (first != last)
  {
  ...
  orange_stl::sort_heap(result_first, result_iter, comp);
  return result_iter;
}
...
// 其余放到 result_false 的输出区间，并返回一个 orange_stl::pair 指向这两个区间的尾部
/*****************************************************************************************/
template <class InputIter, class OutputIter1, class OutputIter2, class UnaryPredicate>
orange_stl::pair<OutputIter1, OutputIter2>
               ...
               UnaryPredicate unary_pred)
{
  ...
  return orange_stl::pair<OutputIter1, OutputIter2>(result_true, result_false);
}
...
void intro_sort(RandomIter first, RandomIter last, Size depth_limit)
{
  while (static_cast<size_t>(last - first) > kSmallSectionSize)
  {
    if (depth_limit == 0)
    {                      // 到达最大分割深度限制
      orange_stl::partial_sort(first, last, last);  // 改用 heap_sort
      return;
    }
    --depth_limit;
    auto mid = orange_stl::median(*(first), *(first + (last - first) / 2), *(last - 1));
    auto cut = orange_stl::unchecked_partition(first, last, mid);
    orange_stl::intro_sort(cut, last, depth_limit);
    last = cut;
  }
}
...
void unchecked_insertion_sort(RandomIter first, RandomIter last)
{
  ...
  {
    orange_stl::unchecked_linear_insert(i, *i);
  }
}
...
void insertion_sort(RandomIter first, RandomIter last)
{
  ...
  for (auto i = first + 1; i != last; ++i)
  {
    ...
    if (value < *first)
    {
      orange_stl::copy_backward(first, i, i + 1);
      *first = value;
    }
    else
    {
      orange_stl::unchecked_linear_insert(i, value);
    }
  }
}
...
void final_insertion_sort(RandomIter first, RandomIter last)
{
  ...
  {
    orange_stl::insertion_sort(first, first + kSmallSectionSize);
    orange_stl::unchecked_insertion_sort(first + kSmallSectionSize, last);
  }
  else
  {
    orange_stl::insertion_sort(first, last);
  }
}
...
void sort(RandomIter first, RandomIter last)
{
  if (first != last)
  {
    ...
    orange_stl::intro_sort(first, last, slg2(last - first) * 2);
    orange_stl::final_insertion_sort(first, last);
  }
}
                ...
                Size depth_limit, Compared comp)
{
  while (static_cast<size_t>(last - first) > kSmallSectionSize)
  {
    if (depth_limit == 0)
    {                            // 到达最大分割深度限制
      orange_stl::partial_sort(first, last, last, comp);  // 改用 heap_sort
      return;
    }
    --depth_limit;
    auto mid = orange_stl::median(*(first), *(first + (last - first) / 2), *(last - 1));
    auto cut = orange_stl::unchecked_partition(first, last, mid, comp);
    orange_stl::intro_sort(cut, last, depth_limit, comp);
    last = cut;
  }
}
                              ...
                              Compared comp)
{
  for (auto i = first; i != last; ++i)
  {
    orange_stl::unchecked_linear_insert(i, *i, comp);
  }
}
...
void insertion_sort(RandomIter first, RandomIter last, Compared comp)
{
  ...
  for (auto i = first + 1; i != last; ++i)
  {
    ...
    {
      orange_stl::unchecked_linear_insert(i, value, comp);
    }
  }
}
...
void final_insertion_sort(RandomIter first, RandomIter last, Compared comp)
{
  if (static_cast<size_t>(last - first) > kSmallSectionSize)
  {
    orange_stl::insertion_sort(first, first + kSmallSectionSize, comp);
    orange_stl::unchecked_insertion_sort(first + kSmallSectionSize, last, comp);
  }
  else
  {
    orange_stl::insertion_sort(first, last, comp);
  }
}
...
void sort(RandomIter first, RandomIter last, Compared comp)
{
  ...
  {
    // 内省式排序，将区间分为一个个小区间，然后对整体进行插入排序
    orange_stl::intro_sort(first, last, slg2(last - first) * 2, comp);
    orange_stl::final_insertion_sort(first, last, comp);
  }
}
                 ...
                 RandomIter last)
{
  ...
  while (last - first > 3)
  {
    auto cut = orange_stl::unchecked_partition(first, last, orange_stl::median(*first,
										  *(first + (last - first) / 2),
										  *(last - 1)));
...
unique_copy(InputIter first, InputIter last, OutputIter result)
{
  if (first == last)
    return result;
  return orange_stl::unique_copy_dispatch(first, last, result, iterator_category(result));
}
...
unique_copy(InputIter first, InputIter last, OutputIter result, Compared comp)
{
  if (first == last)
    return result;
  return orange_stl::unique_copy_dispatch(first, last, result, iterator_category(result), comp);
}
...
{
  first = orange_stl::adjacent_find(first, last);
  return orange_stl::unique_copy(first, last, first);
}
...
{
  first = orange_stl::adjacent_find(first, last, comp);
  return orange_stl::unique_copy(first, last, first, comp);
}
