#ifndef __ORANGE_ALGOBASE_H__
#define __ORANGE_ALGOBASE_H__

// 这个头文件包含了 orange_stl 的基本算法
...
unchecked_copy_cat(RandomIter first, RandomIter last, OutputIter result,
                   orange_stl::random_access_iterator_tag)
{
...
unchecked_copy_backward_cat(BidirectionalIter1 first, BidirectionalIter1 last,
                            BidirectionalIter2 result, orange_stl::bidirectional_iterator_tag)
{
  while (first != last)
    *--result = *--last;
  return result;
...
BidirectionalIter2 
unchecked_copy_backward_cat(BidirectionalIter1 first, BidirectionalIter1 last,
                            BidirectionalIter2 result, orange_stl::random_access_iterator_tag)
{
...
orange_stl::pair<InputIter, OutputIter>
unchecked_copy_n(InputIter first, Size n, OutputIter result, orange_stl::input_iterator_tag)
{
  for (; n > 0; --n, ++first, ++result)
  {
    *result = *first;
  }
  return orange_stl::pair<InputIter, OutputIter>(first, result);
}

template <class RandomIter, class Size, class OutputIter>
orange_stl::pair<RandomIter, OutputIter>
unchecked_copy_n(RandomIter first, Size n, OutputIter result, 
                 orange_stl::random_access_iterator_tag)
{
  auto last = first + n;
  return orange_stl::pair<RandomIter, OutputIter>(last, orange_stl::copy(first, last, result));
}
...
unchecked_move_backward_cat(RandomIter1 first, RandomIter1 last,
                            RandomIter2 result, orange_stl::random_access_iterator_tag)
{
  for (auto n = last - first; n > 0; --n)
    *--result = orange_stl::move(*--last);
  return result;
...
void fill_cat(ForwardIter first, ForwardIter last, const T& value,
              orange_stl::forward_iterator_tag)
{
  for (; first != last; ++first)
  {
    *first = value;
                             ...
                             const unsigned char* last2)
{
  ...
  // 先比较相同长度的部分
  const auto result = std::memcmp(first1, first2, orange_stl::min(len1, len2));
  // 若相等，长度较长的比较大
...
/*****************************************************************************************/
template <class InputIter1, class InputIter2>
orange_stl::pair<InputIter1, InputIter2> 
mismatch(InputIter1 first1, InputIter1 last1, InputIter2 first2)
{
  ...
  return orange_stl::pair<InputIter1, InputIter2>(first1, first2);
}
