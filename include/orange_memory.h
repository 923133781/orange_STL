#ifndef __ORANGE_MEMORY_H__
#define __ORANGE_MEMORY_H__
...
#include "orange_allocator.h"
#include "orange_construct.h"
#include "orange_uninitialized.h"
...
public:
  ...
  ~temporary_buffer()
  {
    orange_stl::destroy(buffer, buffer + len);
    free(buffer);
  }
...
private:
  ...
  { orange_stl::uninitialized_fill_n(buffer, len, value); }
...
temporary_buffer(ForwardIterator first, ForwardIterator last)
{
  try
  {
    len = orange_stl::distance(first, last);
    allocate_buffer();
    if (len > 0)
    {
